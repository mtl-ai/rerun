...
//!
//! This is a port of the `re_video::encode` module from the in-tree
//! `pg/headless_render_to_mp4` branch. It lives here rather than in `re_video`
//! precisely so that this crate needs no changes inside `crates/` — see
//! `NOTES.md`. It depends only on `ffmpeg_sidecar` (already a workspace
//! dependency, used by the *decode* path) and `re_log`.
