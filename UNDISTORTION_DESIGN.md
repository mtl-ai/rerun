...
- **Python SDK**: `LensDistortion` has a hand-written extension
  (`encodings/lens_distortion_ext.py`) so `rr.Pinhole(distortion=…)` actually
  serializes. Coefficients are zero-padded to 8 (a ROS `plumb_bob` `D` of length 5
  can be passed as-is) and `None`/NaN are rejected: the Arrow field is non-nullable,
  and a null inside `coefficients` makes the viewer drop the *whole* Pinhole chunk
  at IPC decode time (`non-nullable child of type Float64 contains nulls not
  present in parent`). Producers building the Arrow struct by hand must zero-fill.
- **Companion branch**: a `foxglove_msgs/msg/CompressedVideo` (ROS2/CDR) parser in
  `re_mcap` lives on `pg/mcap_compressed_video` -- without it no truck bag renders
  its camera natively, so the e2e validation had no subject. It is independent of
  this branch and is merged alongside it in `pg/distortion_plus_render`. Forwards
  into `VideoStream` like the `CompressedImage` h264 path.
