...
namespace rerun::archetypes {
    Pinhole Pinhole::clear_fields() {
        ...
        archetype.distortion =
            ComponentBatch::empty<rerun::components::LensDistortion>(Descriptor_distortion)
                .value_or_throw();
    ...
    Collection<ComponentColumn> Pinhole::columns(const Collection<uint32_t>& lengths_) {
        ...
        if (distortion.has_value()) {
            columns.push_back(distortion.value().partitioned(lengths_).value_or_throw());
        }
    ...
    Collection<ComponentColumn> Pinhole::columns() {
        ...
        if (distortion.has_value()) {
            return columns(std::vector<uint32_t>(distortion.value().length(), 1));
        }
        if (camera_xyz.has_value()) {
...
namespace rerun {
    ...
    Result<Collection<ComponentBatch>> AsComponents<archetypes::Pinhole>::as_batches(
        ...
        if (archetype.distortion.has_value()) {
            cells.push_back(archetype.distortion.value());
        }
